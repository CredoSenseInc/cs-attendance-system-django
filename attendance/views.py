...
def download(request):
    if(request.method == "POST"):
        ...
        try:
            ...
            writer.writerow([])
            writer.writerow(['Name' ,'ID', 'Date' , 'Status', 'In time' ,  'Out time', 'Overtime', 'Total workhour', 'Comment', 'Note'])
            ...
            for i in range (len(data)):
                ...
                try:
                    if  datetime.datetime(data[i].date.year, data[i].date.month, data[i].date.day)  in additional_work_days:
                        print("Add. Work DAY FOUND")
                        note = "Additional Working Day"
                except:
                    pass
